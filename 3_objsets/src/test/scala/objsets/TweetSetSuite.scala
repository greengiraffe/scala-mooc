...
class TweetSetSuite {
  trait TestSets {
    ...
    val set6 = set5.incl(new Tweet("a", "a 2 body", 50))
  }
  ...
  @Test def `filter: a on set5`: Unit =
    new TestSets {
      assertEquals(1, size(set5.filter(tw => tw.user == "d")))
    }
  ...
  @Test def `mostRetweeted: with set6`: Unit =
    new TestSets {
      val mostRetweeted = set6.mostRetweeted
      assert(mostRetweeted.retweets == 50)
    }
