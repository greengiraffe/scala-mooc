...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def iter(tweetSet: TweetSet): TweetList = {
      if (tweetSet.isEmpty) Nil
      else new Cons(tweetSet.mostRetweeted, iter(tweetSet.remove(tweetSet.mostRetweeted)))
    }
    new Cons(mostRetweeted, iter(remove(mostRetweeted)))
  }
