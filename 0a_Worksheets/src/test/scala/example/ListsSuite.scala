 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of negative numbers`: Unit = {
  ...
  @Test def `max of negative numbers`: Unit = {
  ...
  @Test def `max of zeroes`: Unit = {
    assert(max(List(0,0,0)) == 0)
  }

  @Test def `max of positive & negative numbers`: Unit = {
    assert(max(List(-5,5,0)) == 5)
  }
